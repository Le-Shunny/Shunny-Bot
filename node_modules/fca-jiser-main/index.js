...
function buildAPI(globalOptions, html, jar) {
    ...
    if (maybeCookie.length === 0) throw { error: "Appstate - Cookie Yours is broken, please replace it with a new one, or go to incognito browser then log in and try again  !" };

    if (html.indexOf("/checkpoint/block/?next") > -1) log.warn("login", "Phát Hiện CheckPoint - Can't Log In, Try Logout Then Login And Get Back Appstate - Cookie !");

    var userID = maybeCookie[0].cookieString().split("=")[1].toString();
    logger(`Login At ID: ${userID}`, "[ JEKA-DEV ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`The Area Of The Account Is: ${region}`, "[ JEKA-DEV ]");
    } else {
        ...
        if (newFBMQTTMatch) {
            ...
            if (legacyFBMQTTMatch) {
                ...
                log.warn("login", "Can't Get ID Please Try Again !");
                noMqttData = html;
            }
        }
    }
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
    return function(res) {
        ...
        logger("Đang Đăng Nhập...", "[ JEKA-DEV ]");
        return utils
            ...
            .then(function(res) {
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    logger("Bạn Đang Bật 2 Bảo Mật !", "[ JEKA-DEV ]");
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        logger("Confirm From Browser, Logging In...", "[ JEKA-DEV ]");
                                                        if (callback === prCallback) {
                                                            callback = function(err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
             ...
             1: {    
                 "name": "Create Password For Appstate",
                  onRun: async function() {
                     try {
                         rl.question("Please Enter the Password You Want to Set Appstate !", (answer) => {
                             console.log("Okay Your Password Is: " + answer + "Please Remember This Carefully  !");
                         process.env["FBKEY"] = answer;
                             fs.writeFile('../.env', `FBKEY=${answer}`, function (err) {
                                 if (err) {
                                     logger("Creating ENV File Failed !", "[ JEKA-DEV]")
                                     rl.pause();
                                 }
                                 else logger("Successfully Created ENV File !","[ JEKA-DEV ]")
                                 rl.pause();
                             });
                         })
                     }
                         ...
                         console.log(e);
                         logger("An error occurred while trying to create a question =))", "[ JEKA-DEV ]");
                         rl.pause();
                     }
                 }
             },
             2: {
                 "name": "Continue Running Fca Without Encryption AppState",
                  onRun: async function () {
             rl.pause();
                 }
             },
             3: {
                 "name": "Change Password AppState (Comming Soon..)", 
                 onRun: async function () {
                     console.log(chalk.red.bold("I told you it was coming soon >:v"));                        
                 }
             }
         }
...
/*           var axios = require('axios');
        ...
        console.log(e);
        logger("There was an error while trying to create Random Password =))", "[ JEKA-DEV ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger('Not Ready To Decrypt Appstate !', '[ JEKA-DEV ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger('Decrypt Appstate Successfully !', '[ JEKA-DEV ]');
                }
                catch (e) {
                    logger('Replace Appstate!', '[ JEKA-DEV ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger('Replace AppState !', '[ FCA-JISER ]')
        }
    }
    try { 
    ...
    return logger('Replace Appstate !', '[ FCA-JISER ]')
        ...
        console.log(e);
            ...
            mainPromise
                .then(function() {
                    logger('Complete the Login Process !', "[ JEKA-DEV ]");
                        logger('Auto Check Update ...', "[ JEKA-DEV ]");
            ...
            axios.get('https://pastebin.com/raw/S5Hr7QVT').then(async (res) => {
                const localbrand = JSON.parse(readFileSync('./node_modules/fca-jiser-main/package.json')).version;
                    if (localbrand != res.data.version) {
                        log.warn("[ JEKA-DEV ] •",`New Version Available Publish: ${JSON.parse(readFileSync('./node_modules/fca-jiser-main/package.json')).version} => ${res.data.version}`);
                        log.warn("[ JEKA-DEV ] •",`Proceed to Automatically Update to the Latest Version !`);
                            try {
                                execSync('npm install fca-jiser-main@latest', { stdio: 'inherit' });
                                logger("Version Upgrade Successful!","[ JEKA-DEV ]")
                                logger('Restarting...', '[ JEKA-DEV ]');
                            ...
                            logger(`The Version You Are Currently Using: ` + localbrand + ' !', "[ JEKA-DEV ]"); 
