module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Shen",
  description: "Get osumania info through username",
...
module.exports.run = ({ event, api, args, getText }) => {
    ...
    request(`https://lemmmy.pw/osusig/sig.php?colour=pink&uname=${args.join(" ")}&mode=3&countryrank&onlineindicator=undefined`).pipe(fs.createWriteStream(__dirname + `/cache/${event.senderID}-osu.png`)).on("close", () => api.sendMessage({ attachment: fs.createReadStream(__dirname + `/cache/${event.senderID}-osu.png`) }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/${event.senderID}-osu.png`), event.messageID));}
